...
import com.shoppingmall.common.DbHelper;
...
public class OrdersDao {
	...
	public void updateStatusByNumber(String number, int status)throws RuntimeException{
		...
		try{
			...
			qr.update(conn, sql, status, number);
    ...
    public String getProductNameByOrder(Integer id) {
		String sql = "SELECT p.name from product p left join item i on p.id= i.product_id left join orders o ON o.id = i.order_id WHERE o.id = ?";
		Connection conn = null;
        String ret ;
		try{
			conn = DbHelper.getConn();
            ret = qr.query(conn, sql, new ColumnListHandler(), id).toString();
        }catch(Exception e){
			throw new RuntimeException(e);
		}finally{
			DbUtils.closeQuietly(conn);
